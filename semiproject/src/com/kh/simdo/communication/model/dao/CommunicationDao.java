...
public class CommunicationDao {
	...
	/**
	 * @author 김백관 게시글저장/파일업로드
	 */

	// 파일테이블에 파일 정보 저장
	public int insertFile(Connection conn, FileVO fileData) {
		int res = 0;
		String sql = "insert into tb_file " + "(f_idx,origin_file_name,rename_file_name,save_path) "
	...
	public int[] selectPagingByQna(Connection conn, int page, int userNo) {
		...
		try {
			...
			throw new DataAccessException(ErrorCode.PQ01,e);
		}
	...
	public List<Map<String, Object>> selectQnaList(Connection conn, int page, int userNo) {
		...
		try {
			...
			throw new DataAccessException(ErrorCode.SQ01, e);
		} finally {
			jdt.close(rset, pstm);
		}
	...
	public Communication selectQnaByQstnNo(Connection conn, int qstnNo) {
		...
		try {
			...
			throw new DataAccessException(ErrorCode.SQ02, e);
		} finally {
			jdt.close(rset, pstm);
		}
		return comm;
	...
	public int[] selectPagingByNotice(Connection conn, int page) {
		...
		try {
			...
			throw new DataAccessException(ErrorCode.PN01,e);
		}
	...
	public List<Map<String, Object>> selectNoticeList(Connection conn, int page) {
		...
		try {
			...
			throw new DataAccessException(ErrorCode.SN01, e);
		} finally {
			jdt.close(rset, pstm);
		}
	...
	public Notice selectNoticeByNoticeNo(Connection conn, int noticeNo) {
		...
		try {
			...
			throw new DataAccessException(ErrorCode.SN02, e);
		} finally {
			jdt.close(rset, pstm);
		}
		return notice;
